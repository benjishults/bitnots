...
public class MultiSplitPane extends JPanel {
  ...
  /**
   * Sets the value of the <code>continuousLayout</code> property. If true, then
   * the layout is revalidated continuously while a divider is being moved. The
   ...
   * MultiSplitPane. The default DividerPainter only draws the activeDivider (if
   * there is one) and then, only if continuousLayout is false. The value of
   * this property is used by the paintChildren method: Dividers are painted
   * after the MultiSplitPane's children have been rendered so that the
   * activeDivider can appear "on top of" the children.
   * 
